public class NaturalCubicSpline
{
    ...
    public NaturalCubicSpline(double[] x, double[] y)
    {
        ...
        var sorted = x.Zip(y, (xi, yi) => new { X = xi, Y = yi })
                    .OrderBy(p => p.X)
                    .ToArray();
        ...
        for (int i = 1; i < sorted.Length; i++)
        {
            if (sorted[i].X == sorted[i - 1].X)
                throw new ArgumentException($"Duplicate x value found: {sorted[i].X}");
        }

        this.x = sorted.Select(p => p.X).ToArray();
        this.y = sorted.Select(p => p.Y).ToArray();
    ...
    public IEnumerable<(double X, double Y)> RawPoints =>
    x.Zip(y, (xi, yi) => (X: xi, Y: yi));
